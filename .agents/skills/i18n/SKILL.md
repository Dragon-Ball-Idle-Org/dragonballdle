...
2. **New keys**: Add to `pt-BR.json` and `en-US.json` first, then remaining locales every time, don't miss any, and respect the language specified in the file, including different characters like `ja-JP`, `ko-KR`, `zh-CN`, `zh-TW`. This is very important.
